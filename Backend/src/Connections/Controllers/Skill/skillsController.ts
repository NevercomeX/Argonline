...
export async function getAvailableSkills(characterId: any) {
  try {
...
export async function getSkillTreeByJobClassId(jobclass: any) {
  try {
...
export async function levelUpCharacterSkill(characterId: any, skillId: any) {
  try {
    const character = await prisma.character.findUnique({
      where: { id: parseInt(characterId) },
      include: { skills: true },
...
export async function resetCharacterSkills(characterId: any) {
  try {
    ...
    const totalSpent = character.skills.reduce((sum: any, skill: any) => sum + skill.level, 0);
...
export async function learnCharacterSkill(characterId: any, skillId: any) {
  try {
    const character = await prisma.character.findUnique({
      where: { id: parseInt(characterId) },
    });
