 ...
 */
export async function getStorageItems(userId: any) {
  try {
 ...
 */
export async function getStorageItemById(id: any) {
  try {
 ...
 */
export async function addItemToStorage(userId: any, itemId: any, quantity: any, refineLevel = 0, cards = null) {
  try {
    ...
    if (existingItem) {
      ...
      await prisma.storageItem.create({
        data: {
          ...
          refineLevel,
          cards: cards || undefined, // Puede ser null o un objeto JSON, según la lógica de tu aplicación
        },
      });
    }
  } catch (error) {
 ...
 * @returns {Promise<void>}
 */
export async function removeItemFromStorage(userId: any, itemId: any, quantity: any, refineLevel = 0) {
  try {
    const parsedQuantity = parseInt(quantity);
    const existingItem = await prisma.storageItem.findFirst({
      where: {
