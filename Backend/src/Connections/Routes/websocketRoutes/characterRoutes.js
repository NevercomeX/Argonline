...
const broadcastCharacterUpdate = (wss, characterId, action) => {
 ...
 */
export default function characterRoutes(handlers) {
  // Obtener todos los personajes de un usuario con paginación
  handlers["getUserCharacters"] = async (ws, message, wss) => {
  ...
  handlers["getAllCharacters"] = async (ws, message, wss) => {
  ...
  handlers["getCharacterById"] = async (ws, message, wss) => {
    const { id } = message;
    try {
  ...
  handlers["updateCharacter"] = async (ws, message, wss) => {
  ...
  handlers["createCharacter"] = async (ws, message, wss) => {
