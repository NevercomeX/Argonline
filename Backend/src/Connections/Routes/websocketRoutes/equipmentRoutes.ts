...
const broadcastEquipmentUpdate = (wss: any, characterId: any, action: any, slot: any, itemId: any) => {
 ...
 */
export default function equipmentRoutes(handlers: any) {
  // Obtener todos los equipamientos
  handlers["getEquipment"] = async (ws: any, message: any, wss: any) => {
    try {
  ...
  handlers["getEquipmentById"] = async (ws: any, message: any, wss: any) => {
    const { id } = message;
    try {
  ...
  handlers["getEquipmentSlotsByCharacterId"] = async (ws: any, message: any, wss: any) => {
  ...
  handlers["getEquipmentMenu"] = async (ws: any, message: any, wss: any) => {
    const { characterId } = message;
    try {
  ...
  handlers["unequipItem"] = async (ws: any, message: any, wss: any) => {
  ...
  handlers["equipItem"] = async (ws: any, message: any, wss: any) => {
  ...
  handlers["getEquipmentSlot"] = async (ws: any, message: any, wss: any) => {
    const { characterId, slotType } = message;
    try {
      ...
      ws.send(
        JSON.stringify({
          type: "error",
          error: `Error al obtener equipamiento: ${(error as any).message}`,
        })
      );
    }
  };
