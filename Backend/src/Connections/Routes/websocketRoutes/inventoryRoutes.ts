...
const broadcastInventoryUpdate = (wss: any, characterId: any, action: any, itemId: any, quantity: any) => {
 ...
 */
export default function inventoryRoutes(handlers: any) {
  // Obtener inventario completo de un personaje
  // Se espera que el mensaje contenga: { type: "getInventory", characterId }
  handlers["getInventory"] = async (ws: any, message: any, wss: any) => {
  ...
  handlers["getInventoryItem"] = async (ws: any, message: any, wss: any) => {
    const { id } = message;
    try {
  ...
  handlers["addItemToInventory"] = async (ws: any, message: any, wss: any) => {
    ...
    try {
      ...
      ws.send(
        JSON.stringify({
          type: "error",
          error: `Error al añadir ítem al inventario: ${(error as any).message}`,
        })
      );
    }
  };
  ...
  handlers["removeItemFromInventory"] = async (ws: any, message: any, wss: any) => {
    const { characterId, itemId, quantity } = message;
    try {
      ...
      ws.send(
        JSON.stringify({
          type: "error",
          error: `Error al eliminar ítem del inventario: ${(error as any).message}`,
        })
      );
    }
  };
  ...
  handlers["getStorageItem"] = async (ws: any, message: any, wss: any) => {
    const { id } = message;
    try {
      ...
      ws.send(
        JSON.stringify({
          type: "error",
          error: `Error al obtener el inventario del personaje: ${(error as any).message}`,
        })
      );
    }
  };
