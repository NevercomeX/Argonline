 ...
 */
export default function itemRoutes(handlers) {
  // Handler para obtener todos los ítems (con paginación)
  handlers["getAllItems"] = async (ws, message, wss) => {
    try {
  ...
  handlers["getItemById"] = async (ws, message, wss) => {
    const { id } = message;
    try {
  ...
  handlers["getItemNameById"] = async (ws, message, wss) => {
    const { id } = message;
    try {
