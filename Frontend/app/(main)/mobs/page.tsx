...
const MonsterList: React.FC = () => {
  ...
  const fetchMonsters = async (pageNumber: number) => {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_CHAR_URL}/mobs?page=${pageNumber}`,
    );
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {monsters.map((monster) => (
          <div
            key={monster.id}
            className="border border-gray-300 bg-white p-4 rounded-lg shadow hover:shadow-md hover:bg-gray-100"
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                <span className="font-medium">magicPower:</span>{" "}
                {monster.magicPower}
              </div>
              <div>
              ...
              <div>
                <span className="font-medium">Defensa Magica:</span>{" "}
                {monster.magicDefense}
              </div>
              <div>
                <span className="font-medium">Exp:</span>{" "}
                {monster.giveBaseExpAmount}
              </div>
              <div>
                <span className="font-medium">Job Exp:</span>{" "}
                {monster.giveJobExpAmount}
              </div>
              <div>
